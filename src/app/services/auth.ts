...
export class AuthService {
  ...
  async login(payload: any) {
    const res = await this.api.post<{ token: string, user: User }>('/api/v1/auth/login', payload);
  ...
  async register(payload: any) {
    const res = await this.api.post<{ token: string, user: User }>('/api/v1/auth/register', payload);
    localStorage.setItem('token', res.token);
    this.userSignal.set(res.user);
    return res.user;
  }
